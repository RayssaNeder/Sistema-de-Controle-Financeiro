...
public interface MovimentacaoApi {
    ...
    @Operation(summary = "Obter movimentação por UUID",
               responses = {
    ...
    @Operation(summary = "Listar movimentações por cliente",
               responses = {
    ...
    @Operation(summary = "Atualizar movimentação por UUID",
               responses = {
    ...
    @PutMapping("/{uuid}")
    ResponseEntity<MovimentacaoDTO> updateMovimentacao(@PathVariable UUID uuid, @RequestBody UpdateMovimentacao updateMovimentacao);

    @Operation(summary = "Deletar movimentação por UUID",
               responses = {
    ...
    @Operation(summary = "Listar movimentações com paginação",
               responses = {
                   @ApiResponse(responseCode = "200", description = "Lista de movimentações retornada com sucesso",
                                content = @Content(schema = @Schema(implementation = PageMovimentacaoDTO.class)))
               })
    @GetMapping
