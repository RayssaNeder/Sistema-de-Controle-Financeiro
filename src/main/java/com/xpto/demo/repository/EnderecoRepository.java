...
public interface EnderecoRepository extends JpaRepository<EnderecoDomainEntity, UUID> {
	...
	@Query("SELECT new com.xpto.demo.dto.EnderecoDTO(" + " e.uuid, " + " e.rua, " + "e.bairro, " + "	 e.cep, "
			+ "	 e.numero, " + "	 e.complemento, " + "e.cidade, " + "	e.uf , " + "	cl.uuid ) "
			+ "FROM EnderecoDomainEntity e " + "JOIN ClienteDomainEntity cl ON cl.id = e.cliente.id "
