...
public interface RelatorioRepository extends JpaRepository<MovimentacaoDomainEntity, Long> {
	...
	@Query("""
			    ...
			    GROUP BY cl.nome, cl.dataCadastro,
			             e.rua, e.numero, e.complemento, e.bairro, e.cidade, e.uf, e.cep
			""")
	RelatorioSaldoClienteDTO gerarRelatorioSaldoClienteByPeriodo(@Param("uuidCliente") UUID uuidCliente,
	...
	@Query("""
		    SELECT new com.xpto.demo.dto.RelatorioReceitaDTO(
		        ...
		        COUNT(m),
		        CASE
		            WHEN COUNT(m) <= 10 THEN CAST(COUNT(m) * 1.00 AS BigDecimal)
		            WHEN COUNT(m) <= 20 THEN CAST(COUNT(m) * 0.75 AS BigDecimal)
		            ELSE CAST(COUNT(m) * 0.50 AS BigDecimal)
		        END,
		        NULL
		    )
		    FROM MovimentacaoDomainEntity m
		    ...
		    GROUP BY cl.nome
		""")
		List<RelatorioReceitaDTO> gerarRelatorioReceita(@Param("dataInicio") LocalDate dataInicio,
