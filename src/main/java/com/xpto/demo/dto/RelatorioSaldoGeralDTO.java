...
public class RelatorioSaldoGeralDTO implements RelatorioComFormato {
	...
	public RelatorioSaldoGeralDTO(String nomeCliente, LocalDate dataCadastro, LocalDate dataReferencia,
			BigDecimal saldo) {
	...
	public String gerarTextoRelatorio() {
		...
		sb.append(String.format("Cliente: %s - Cliente desde: %s - Saldo em %s: %s\n", nomeCliente,
				dataCadastro.format(formatter), dataReferencia.format(formatter), String.format("%,.2f", saldo)));
	...
	public String formatarLinhaRelatorio() {
		...
		return String.format("Cliente: %s - Cliente desde: %s - Saldo em %s: %s", nomeCliente, cadastroFormatado,
				referenciaFormatada, saldoFormatado);
	}
}
